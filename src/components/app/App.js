...
import {filterRaidDriver} from "../../redux/RaidDriver";
            ...
            websocket.onmessage = (event) => {
                ...
                if (data.message.status) {
                    ...
                    if (data.message.status === "confirmed" || data.message.status === "Added") {
                        ...
                        let alert = {
                            id: idAlert, text: t("alert1"), img: "./images/green.png",
                        };
                    ...
                    if (data.message.status === "canceled") {
                        let idAlert = Date.now();
                        let alert = {
                            id: idAlert, text: t("alert2"), img: "./images/red.png",
                        };
                        dispatch(addAlert(alert));
                        setTimeout(() => {
                            dispatch(delAlert(idAlert));
                    ...
                    if (data.message.status === "Accepted") {
                        let idAlert = Date.now();
                        let alert = {
                            id: idAlert, text: t("alert8"), img: "./images/green.png",
                        };
                        dispatch(addAlert(alert));
                        setTimeout(() => {
                            dispatch(delAlert(idAlert));
                    ...
                    if (data.message.status === "delivering") {
                        let idAlert = Date.now();
                        let alert = {
                            id: idAlert, text: t("alert9"), img: "./images/green.png",
                        };
                        dispatch(addAlert(alert));
                        setTimeout(() => {
                            dispatch(delAlert(idAlert));
                    ...
                    if (data.message.status === "delivered") {
                        let idAlert = Date.now();
                        let alert = {
                            id: idAlert, text: t("alert10"), img: "./images/green.png",
                        };
                        dispatch(addAlert(alert));
                        setTimeout(() => {
                            dispatch(delAlert(idAlert));
                        ...
                        console.log(newRaidDriver)
                        console.log("driver----", drivers)
                        console.log(Number(data.message.id))

                        // dispatch(addRaidDriver(newRaidDriver));
                        dispatch(filterDriverDelivered(data.message.id));
                        // dispatch(updateActiveDriver(newRaidDriver.driver.id));
                    }
    ...
    useEffect(() => {
        ...
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            websocket.onerror = (event) => {
                let alert = {
                    id: idAlertError, text: "Internet kuchsiz!", img: "./images/red.png",
                };
                dispatch(addAlert(alert));
                setTimeout(() => {
                    window.location.reload()
