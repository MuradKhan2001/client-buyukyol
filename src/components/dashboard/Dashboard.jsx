...
import { addRaidDriver, updateRaidDriver } from "../../redux/RaidDriver";
import {
  updateActiveDriver,
  addActiveDriver,
} from "../../redux/ActiveDriversList";
...
navigator.geolocation.getCurrentPosition((position) => {
  ...
  websocket = new WebSocket(
    `wss://api.buyukyol.uz/ws/orders/${location}/?token=${localStorage.getItem(
      "token"
    )}`
  );
});
...
const Dashboard = () => {
  ...
  const activeDriversList = useSelector((store) => store.ActiveDriversList.data);
      ...
      websocket.onmessage = (event) => {
        ...
        if (data.message.status) {
          ...
          if (data.message.status === "canceled") {
            ...
            let driver = drivers.filter(
              (item) => item.order_id !== data.message.order_id
            );

            if (data.message.driver_id){
                let activeDriver = activeDriversList.filter(
                  (item) => item.driver !== data.message.driver_id
                );
                dispatch(updateActiveDriver(activeDriver));
            }
          ...
          if (data.message.status === "delivered") {
            ...
            let RaidDriver = drivers.filter((item) => item.id === data.message.id);
            dispatch(addRaidDriver(RaidDriver));

            let driversList= drivers.filter((item) => item.id !== data.message.id);
            dispatch(updateDriver(driversList));

            let activeDrivers= activeDriversList.filter((item) => item.driver !== RaidDriver.driver.id);
            dispatch(updateActiveDriver(activeDrivers));
          }

          if (data.message.status === "location") {
            console.log(data.message.driver[0]);
            dispatch(addActiveDriver(data.message.driver[0]));
          }
  ...
  return (
    <webSockedContext.Provider value={sockedContext}>
